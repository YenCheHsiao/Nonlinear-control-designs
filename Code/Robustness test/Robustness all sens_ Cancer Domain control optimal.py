# -*- coding: utf-8 -*-
"""
Created on Sat Oct 28 10:45:06 2023
...
for itr in range(0, 1060):# run 1060 times
    ...
    for i in range(1,len(t)):
        ...
        u = lamb*(x10-xdP[9])-x10d
        if u > 0:
            u = 0
        error = np.sqrt(np.sum(np.power(np.asarray(x)-np.asarray(xdP),2))/len(xdP))
        log.append([t[i],x1,x2,x3,x4,x5,x6,x7,x8,x9,x10,x11,x12,x13,x14,x15,x16,x17,x18,x19,x20,x21,x22,x23,x24,x25,x26,x27,x28,x29,x30,x31,x32,error,u])
...
np.save('DC_sens_robust_trjs', np.asarray(all_log))
# a = np.load('DC_robust_trjs.npy')
# -*- coding: utf-8 -*-
"""
Created on Sun Oct 22 15:15:39 2023
...
def qplot(logno,log1,logtB1,logBlack1,log2,logtB2,logBlack2,log3,logtB3,logBlack3,logBlackU1,logBlackU2,logBlackU3, t):
    ...
    plt.grid()
    plt.legend(['Apoptosis target','Cancerous (no control)','$K_D$ = '+str(Kd1), '$K_D$ = '+str(Kd2), '$K_D$ = '+str(Kd3)], fontsize=20, loc='lower right')
    ...
    for i in range(0,len(logBlack1)):
        if i == 0:
            plt.plot(logtB1[i][-1],logBlackU1[i][-1],'x',color = '#4CAF50', ms=10, markeredgewidth=3, label='$K_D$ = '+str(Kd1))
        else:
            plt.plot(logtB1[i][-1],logBlackU1[i][-1],'x',color = '#4CAF50', ms=10, markeredgewidth=3)
    for i in range(0,len(logBlack2)):
        if i == 0:
            plt.plot(logtB2[i][-1],logBlackU2[i][-1],'o',color = '#EF6C00', ms=10, markeredgewidth=3, fillstyle='none', label='$K_D$ = '+str(Kd2))
        else:
            plt.plot(logtB2[i][-1],logBlackU2[i][-1],'o',color = '#EF6C00', ms=10, markeredgewidth=3, fillstyle='none')
    for i in range(0,len(logBlack3)):
        if i == 0:
            plt.plot(logtB3[i][-1],logBlackU3[i][-1],'+',color = '#1E90FF', ms=10, markeredgewidth=3, fillstyle='none', label='$K_D$ = '+str(Kd3))
        else:

# -*- coding: utf-8 -*-
"""
Created on Sun Oct 22 15:03:16 2023
...
for i in range(1,len(t)):
    ...
    log1.append([t[i],x1,x2,x3,x4,x5,x6,x7,x8,x9,x10,x11,x12,x13,x14,x15,x16,x17,x18,x19,x20,x21,x22,x23,x24,x25,x26,x27,x28,x29,x30,x31,x32,error,u])
...
def qplot(logno, log,log2, t):
    ...
    plt.ylabel(r'$x_6$', fontsize=40)
    plt.xlabel('Time [a.u.]', fontsize=30)
    plt.xticks(np.arange(0, max(log[0])+1, 5), fontsize=20)
    plt.yticks(fontsize=20)
    plt.grid()
    plt.legend(['Apoptosis target','Cancerous (no control)', 'Sigmoid-based controller', 'Domain control'], fontsize=20, loc='lower right')
    ...
    #plt.title('nutlin input')
    plt.ylabel(r'$x_{10}$', fontsize=40)
    plt.xlabel('Time [a.u.]', fontsize=30)
    plt.xticks(np.arange(0, max(log[0])+1, 5), fontsize=20)
    plt.yticks(fontsize=20)
    plt.grid()
    ...
    #plt.title('nutlin input')
    plt.ylabel(r'$u_{10}$', fontsize=40)
    plt.xlabel('Time [a.u.]', fontsize=30)
    plt.xticks(np.arange(0, max(log[0])+1, 5), fontsize=20)
    plt.yticks(fontsize=20)
    #plt.legend(['Lyap: u$_{10}$', 'DI: u$_{10}$'])
    plt.grid()
    plt.tight_layout()
...
print(u_sum_SC, error_sum_SC, u_sum_DI, error_sum_DI)
# 2.8282944911725565 3.3311124114809423 2.867283554233822 3.302877298532405
# -*- coding: utf-8 -*-
"""
Created on Sun Oct 22 15:17:39 2023
...
for i in range(1,len(t)):
    ...
    if t[i-1] == j*T:
        ...
        u = Kp1*e10+Ki1*esum+Kd1*(e10-e10cp)/T
        x10 = x10 + u
        e10cp = xeA[9]-x10
        logMark1.append(i-2)
        logtB1.append([t[i-2],t[i-1]])
...
for i in range(1,len(t)):
    ...
    if t[i-1] == j*T:
        ...
        # Discrete time integral (Trapezoidal)
        esum = esum + (e10+e10cp)/2*T
        u = Kp2*e10+Ki2*esum+Kd2*(e10-e10cp)/T
        x10 = x10 + u
        e10cp = xeA[9]-x10
        logMark2.append(i-2)
        logtB2.append([t[i-2],t[i-1]])
        logBlack2.append([x10p,x10])
        logBlackU2.append([0,u])
    error = np.sqrt(np.sum(np.power(np.asarray(x)-np.asarray(xeA),2))/len(xeA))
...
def qplot(logno,log1,logtB1,logBlack1,log2,logtB2,logBlack2,log3,logtB3,logBlack3,logBlackU1,logBlackU2,logBlackU3, t):
    ...
    plt.plot(log3[0],log3[6],linewidth = 3, color = '#1E90FF',linestyle = 'dashdot')
    ...
    plt.grid()
    plt.legend(['Apoptosis target','Cancerous (no control)','$K_I$ = '+str(Ki1), '$K_I$ = '+str(Ki2), '$K_I$ = '+str(Ki3)], fontsize=20, loc='lower right')
    ...
    plt.plot(log2[0],log2[10],'o', color = '#EF6C00', markerfacecolor='none', ms=10, linewidth = 3, markeredgewidth=3, markevery = logMark2, linestyle = 'dashed', fillstyle='none')
    for i in range(0,len(logBlack2)):
        plt.plot(logtB2[i],logBlack2[i],'#8B4513', linewidth = 2)
    plt.plot(log3[0],log3[10],'+',color = '#1E90FF', markerfacecolor='none', ms=10, linewidth = 3, markeredgewidth=3, markevery = logMark3, linestyle = 'dashdot', fillstyle='none')
    ...
    plt.plot(log3[0],log3[-2],linewidth = 3, color = '#1E90FF',linestyle = 'dashdot')
    #plt.title('nutlin input')
    ...
    # single line
    for i in range(0,len(logBlack1)):
        # plt.plot(logtB1[i],logBlackU1[i],color = '#4CAF50', linewidth = 3)
        if i == 0:
            plt.plot(logtB1[i][-1],logBlackU1[i][-1],'x',color = '#4CAF50', ms=10, markeredgewidth=3, label='$K_I$ = '+str(Ki1))
        else:
            plt.plot(logtB1[i][-1],logBlackU1[i][-1],'x',color = '#4CAF50', ms=10, markeredgewidth=3)
    for i in range(0,len(logBlack2)):
        # plt.plot(logtB2[i],logBlackU2[i],color = '#EF6C00', linewidth = 3,linestyle = 'dashed')
        if i == 0:
            plt.plot(logtB2[i][-1],logBlackU2[i][-1],'o',color = '#EF6C00', ms=10, markeredgewidth=3, fillstyle='none', label='$K_I$ = '+str(Ki2))
        else:
            plt.plot(logtB2[i][-1],logBlackU2[i][-1],'o',color = '#EF6C00', ms=10, markeredgewidth=3, fillstyle='none')
    for i in range(0,len(logBlack3)):
        # plt.plot(logtB3[i],logBlackU3[i],color = '#1E90FF', linewidth = 3,linestyle = 'dashdot')
        if i == 0:
            plt.plot(logtB3[i][-1],logBlackU3[i][-1],'+',color = '#1E90FF', ms=10, markeredgewidth=3, fillstyle='none', label='$K_I$ = '+str(Ki3))
        else:
            plt.plot(logtB3[i][-1],logBlackU3[i][-1],'+',color = '#1E90FF', ms=10, markeredgewidth=3, fillstyle='none')
            #plt.plot(logtB1[i],log1[-1][i],color = '#4CAF50', linewidth = 3)
    # for i in range(0,len(logBlack2)):
    #      plt.plot(log2[0][i],log2[-1][i],'#EF6C00', linewidth = 3)
    # for i in range(0,len(logBlack3)):
    #     plt.plot(log3[0][i],log3[-1][i],'#1E90FF', linewidth = 3)
    #plt.vlines(x = 37, ymin = 0, ymax = max(xs),colors = '#4CAF50',label = 'vline_multiple - full height')
...
Linewidth_nonL = 3
qplot(outo,log1,logtB1,logBlack1,log2,logtB2,logBlack2,log3,logtB3,logBlack3,logBlackU1,logBlackU2,logBlackU3,t)
plt.show()